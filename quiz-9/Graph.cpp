...
bool Graph::isBridge(int key1, int key2)
{
  ...
  // Step1: Compute initial_components. Implement and use the DFTraversal function.
  for(int i = 0; i < vertices.size(); i++){
  ...
  // Step 2: Remove the edge (key1,key2) from the graph, modify the adjacency list(s) in the removeEdge function.
  ...
  // Step 3: Compute components_after_removal (use DFTraversal again)
  for(int i = 0; i < vertices.size(); i++){
    if(!vertices[i]->visited){
      DFTraversal(vertices[i]);
  ...
  // Step5: check if the no of connected componenets increases after removing the edge and return true
