...
Node* BST::deleteNode(Node *currNode, int value)
{
  ...
  else
  {
    ...
    else
    {
      ...
      currNode->left = deleteNode(currNode->left, min->key);
    }
  }
return currNode;
}
...
// GOLD TODO
bool BST::isValidHelper(Node* currNode){
  if(currNode == NULL){
    return true;
  }
  if(currNode->right != NULL && currNode->right->key < currNode->key){
    return false;
  }
  if(currNode->left != NULL && currNode->left->key > currNode->key){
    return false;
  }

  return isValidHelper(currNode->right) && isValidHelper(currNode->left);
}
bool BST::isValidBST()
{
